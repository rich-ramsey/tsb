...
## **376-1309-00: The Social Brain: Critical Perspectives on Science, Society and Neurodiversity**
...
raw_data <- tribble(
  ~Week, ~Date, ~Title, ~Link, ~Description,
  ...
  3, "Sep 30", "Crisis Part 2", "../lectures/02_crisis.html", "The reform movement in psychological science",
  4, "Oct 7", "Crisis Part 3 (Recorded lecture)", "../lectures/02_crisis.html", "Are there problems in psychological theory?",
  5, "Oct 14", "Crisis Part 4 (No lecture - reflective writing)", "../lectures/02_crisis.html", "Does the crisis extend beyond psychology?",
...
schedule <- raw_data %>% 
  mutate(
    # Create different table styling based on format
    Lecture = if (knitr::is_html_output()) {
      cell_spec(
        Title,
        "html", 
        link = Link
      ...
      # For PDF output, just use the title without links
      Title
    }
  ) %>% 
  select(Week, Date, Lecture, Description)

# Render table differently based on output format
if (knitr::is_html_output()) {
  schedule %>% 
    ...
    column_spec(1:2, width = "5em", bold = TRUE) %>%
    column_spec(3, width = "20em") %>%
    column_spec(4, width = "30em")
} else {
  # Very simplified table for PDF output to avoid dimension errors
  schedule %>% 
